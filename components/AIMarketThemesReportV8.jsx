...
export default function AIMarketThemesReportV8() {
  ...
  const stockData = {
    ...
    powerGen: [
      ...
      { ticker: "MOD", name: "Modine", mktCap: 4500, revGr: 15.0, m1: 20.0, m3: 35.0, m6: 55.0, ps: 1.5 },
  ...
  const highConvictionThemes = [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        { h: "I. Datacenters Are Becoming Power Companies", t: "In Texas, somewhere between 1% and 5% of requested megawatt-scale grid connections get approved on time. Queue times stretch two to five years. AI deployment cycles? Quarterly. Shockingly, despite the long queue, Texas is the best case scenario. Texas has become ground zero for AI infrastructure because of a quirk in American energy regulation: it runs its own grid." },
        { h: "The ERCOT Advantage", t: "Texas operates the only major U.S. grid that's almost entirely isolated from federal oversight. ERCOT (the Electric Reliability Council of Texas) isn't subject to FERC jurisdiction, which means faster permitting, more flexible interconnection rules, and a deregulated market where power buyers can negotiate directly with generators. When waiting for the grid costs billions in forgone earnings, operators stop waiting. They build their own power plants. For datacenter operators trying to build behind-the-meter generation, this is gold. You can cut deals that would take years to approve elsewhere. In Texas, datacenter operators get cheap land, existing energy infrastructure (pipelines, NG, & renewables), and it has no state income tax." },
        { h: null, t: "When waiting for the grid costs billions in forgone earnings, operators stop waiting. They build their own power plants. This isn't metaphor. OpenAI and Oracle jointly ordered 2.3 gigawatts of gas turbines in Texas. xAI deployed 100,000 GPUs with 500 megawatts of mobile gas generation—in four months. These aren't datacenters with backup generators. They're private power plants that happen to run AI. The industry calls them \"behind-the-meter\" (BTM) solutions—meaning the power never touches the public grid. Operators control provisioning, permitting, and pricing. More than 80% of new AI datacenters are now pursuing BTM. Orders are expected to exceed 10 gigawatts by mid-2026, more than double the prior year. As Dylan Patel of SemiAnalysis put it in December: \"The grid is sold out. Nobody's waiting on chips—they're waiting on power. The people who win are the ones who can get megawatts fastest.\" The competitive moat in AI infrastructure has shifted in 2026. Forget land, fiber, or cooling. The new moat is turbine lead time. Fuel access. Regulatory arbitrage. The companies that figure out how to generate their own electricity—fast—will capture that $10B-per-gigawatt prize. Everyone else will wait in queue." },
        { h: "If You Can't Expand the Grid: Increase Compute per Megawatt", t: "There is a second way to beat the power bottleneck that is easy to miss: increase the share of delivered electricity that actually reaches the IT load. Data centers have to spend power on cooling, pumps, fans, and power conversion before a single GPU does useful work. The industry tracks this with PUE (Power Usage Effectiveness), which is simply total facility power divided by IT power. Lower PUE means more of every megawatt becomes compute. That makes cooling efficiency a direct lever on capacity. Better thermal design and more efficient cooling systems can free up meaningful \"virtual capacity,\" letting operators deploy more compute per constrained grid connection and improve unit economics even when absolute megawatts are capped. In a world where the grid is sold out, the winners are not only the companies that secure megawatts fastest, but also the ones that convert each megawatt into the most billable inference." },
        { h: "II. The Capacity Is Built. The Revenue Isn't.", t: "That $10-$12 Billion in revenue per gigawatt only materializes if the datacenter is being used for inference—meaning production workloads that customers pay for. Training generates no revenue. It's R&D. Capex disguised as OpEx. Right now, most of the shiny new infrastructure is training, not earning. Gavin Baker framed this bluntly on a16z's Runtime podcast: \"Those Blackwells are being used for training, and there's no ROI on training. The ROI comes from inference.\" This creates what you might call an ROIC air gap. Capital has been deployed. Operating costs are climbing. Revenue hasn't started. The sector is entering a two-to-three quarter window where return metrics will deteriorate even as investment accelerates. This isn't a bubble in the traditional sense. It's a timing mismatch with a measurable downside. The question isn't whether AI infrastructure will generate returns—it's whether your cost of capital can survive the wait." },
        ...
        { h: "IV. How the Electric Grid Impacts Software Margins (hint = inference)", t: null },
        { h: "What Most Investors Get Wrong About AI", t: "Traditional SaaS companies often have gross margins above 75% because serving the ten-thousandth customer adds almost no incremental cost. Many investors assume AI applications work the same way, just with smarter features bolted on. That assumption is expensive to get wrong." },
        { h: "Understanding AIaaS Cost Structure is Essential", t: "Every time an AI model generates an answer, summary, or recommendation, it performs a computation called inference—and inference consumes real resources: GPU cycles, <b><i>electricity</i></b>, and often third-party API fees. The meter runs with every query. This makes AI economics structurally closer to a utility than to software: usage scales, and so do costs. Some AI-native companies already report gross margins 20-30% below their pure-software predecessors, precisely because customers are actually using the AI features. The counterintuitive result is that engagement—normally a sign of product strength—can pressure profitability if pricing doesn't keep pace with inference spend. The companies that will compound value are not the ones with the most impressive demos; they are the ones engineering their way to lower cost-per-task while usage grows. That's a fundamentally different game than the SaaS era trained investors to play. This explains why the 1 year returns of public software companies are DOWN over -28% on average, while semis, computer hardware, and electronic components have done very well.", isHTML: true },
        { h: "The Struggle Between Legacy SaaS Companies & Emerging AIaaS", t: "This creates an innovator's dilemma hiding in plain sight. Public SaaS companies are valued on margin expansion narratives, which rewards them for adding \"AI features\" that don't actually drive much inference cost—impressive in demos, thin in production. Meanwhile, private vertical AI startups face no such constraint. They can accept 50-60% gross margins and deliver AI that genuinely works because they're not optimizing around a quarterly earnings story. They're optimizing around winning the customer. The risk for incumbents isn't losing on technology—most can access the same foundation models. The risk is losing on willingness: match the product and destroy the margin story, or protect margins and slowly leak share." },
        { h: "What Public Equity Investors Should Look For in Software", t: "For public equity investors, this reframes what to look for. The durable winners are the companies engineering their way to lower cost-per-task as the product scales—verticalization through smaller specialized models, shorter prompts, caching, and pushing routine logic back into deterministic code. The tell is whether management talks about AI as a margin opportunity (defensive, probably under-delivering) or a market expansion opportunity (willing to trade margin for share). That distinction will separate the compounders from the incumbents running out the clock." },
    ...
    return (
      <div style={{ maxWidth: "960px", margin: "0 auto 32px", padding: "24px", background: p.surface1, borderRadius: "12px", border: `1px solid ${p.border}`, boxShadow: "0 8px 24px rgba(17, 24, 39, 0.08)" }}>
        <div style={{ marginBottom: "16px" }}>
          <h4 style={{ ...s.h4, marginBottom: "4px" }}>When Early Control Becomes Returns</h4>
  ...
  const renderSection01 = () => (
    <section style={s.section}>
      ...
      <div style={s.mb32}>
        <p style={s.body}>In November 2022, OpenAI released ChatGPT to the public. Within weeks, artificial intelligence went from research curiosity to the most important investment theme in a generation. The tech giants—Apple, Microsoft, Alphabet, Amazon, and Meta—would go on to push the S&P 500 steadily higher over the next two years.</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginBottom: '32px' }}>
          {theme.paragraphs.map((para, i) => (
            <div key={i}>
              {para.h && <h4 style={s.h4}>{para.h}</h4>}
              {para.isHTML ? (
                <p style={s.body} dangerouslySetInnerHTML={{ __html: para.t }} />
              ) : (
                <p style={s.body}>{para.t}</p>
              )}
            </div>
